...
## Evaluation hooks
Cases in `evals/evals.json` carrying `"command": "create-animatic"`, run with `node tools/run-evals.ts --skill video-production --command create-animatic`: a `draft` case where timing uncertainty warrants an animatic, a control case where it does not and must be skipped, and `boundary-animatic-frame-not-a-reference`, which refuses to promote an animatic frame to a reference frame. Structure is checked by `tools/validate-repo.ts`; preview construction runs through `scripts/render-timeline.ts`, though neither case carries a deterministic check that executes it.
