...
## Evaluation hooks
`evals/evals.json` cases tagged `"command": "define-direction"`, run with `node tools/run-evals.ts --skill video-production --command define-direction`. One case, `draft-uncertain-direction`, covers ambiguous direction (cheap artifact first, explicit selection, no premature motion). Required coverage, not yet written: a control case where the brief is already specific and no direction alternatives should be generated. `tools/validate-production.ts` catches a direction marked approved with no approver recorded, through its `approval-without-approver` rule.
